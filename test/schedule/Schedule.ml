...
let mk_schedule test_name expect_sched typing_env deps queries clause =
  Alcotest.check
    Testable.schedule
    test_name
    expect_sched
    M.(
      eval ~init:typing_env
      @@ bind ~f:(fun _ -> ScheduleM.of_clause clause)
      @@ ScheduleM.solve ~deps queries)
;;
...
let vx = Core.(Schedule.Var.Named (Tmvar.from_string "x"))
let vy = Core.(Schedule.Var.Named (Tmvar.from_string "x"))

let sched_rsg =
  Schedule.
    { head_vars = [ 0, vx; 1, vy ]; graph = Tree.leaf (Vertex.make [], []) }
;;

let rsg_schedule () =
  Programs.RSG.(
    mk_schedule "RSG schedule" sched_rsg tyenv deps queries cl_rsg_rec)
;;
...
let test_cases =
  Alcotest.
    ...
    ; test_case
        ...
        eff_bad_constraint
      (* ; test_case
         "RSG recursive schedule after solving"
         `Quick
         rsg_schedule
      *)
    ]
