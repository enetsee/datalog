...
module type S0 = sig
  ...
  module Effect (M : sig
    ...
    val transform_atom : t -> f:(Atom.t -> Atom.t M.t) -> t M.t
    val transform_head : t -> f:(Subgoal.t -> Subgoal.t M.t) -> t M.t
    val transform_body : t -> f:(Subgoal.t -> Subgoal.t M.t) -> t M.t
