...
type t =
  { clauseFwd : int Clause.Raw.Map.t
  ; clauseBwd : Clause.Raw.t Int.Map.t
...
let predClauses ~predFwd Program.Raw.{ clauses; _ } =
  let ungrouped, clidxs =
    List.unzip
    @@ List.mapi clauses ~f:(fun idx (Clause.Raw.{ head; _ } as cl) ->
           (Pred.Map.find_exn predFwd (Lit.Raw.pred_of head), idx), (cl, idx))
  in
  let fwd = Clause.Raw.Map.of_alist_exn clidxs
...
(** For each intensional predicate, a list of predicates in which it appears as a literal
      ...
      r => { (Neg,p) , (Pos,q) } 
  *)
let predEdges ~predFwd ~predBwd (Program.Raw.{ clauses; _ } as prog) =
  let itnls = Program.Raw.intensionals prog in
  Int.Map.of_alist_exn
  ...
  List.concat_map clauses ~f:(fun Clause.Raw.{ head; body; _ } ->
      let dest = Pred.Map.find_exn predFwd (Lit.Raw.pred_of head) in
      List.filter_map
        ~f:(fun Lit.Raw.{ pred; pol; _ } ->
...
let from_program prog =
  let pidxs = List.mapi ~f:(fun idx p -> p, idx) @@ Program.Raw.preds_of prog in
...
let pp_clauseFwd ppf clauseFwd =
  Fmt.(vbox @@ list ~sep:cut @@ pair ~sep:(always " => ") Clause.Raw.pp int) ppf
  @@ Clause.Raw.Map.to_alist clauseFwd
;;
