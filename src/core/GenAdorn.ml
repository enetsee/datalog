...
let adorn_clause cl ~bpatt ~ord =
  let region = Raw.Clause.region_of cl in
  match ord bpatt cl with
  | Some raw_clause ->
    let head, bound = adorn_head_lit ~bpatt @@ Raw.Clause.head_of raw_clause in
    let body = adorn_body ~bound @@ Raw.Clause.body_of raw_clause in
    Ok Adorned.Clause.(clause ~region head body)
...
module WorkItem = struct
  module X = struct
    ...
    let from_clause Adorned.Clause.{ body; _ } =
...
let apply prog =
  ...
  let queries = Raw.Program.queries_of prog in
  ...
  let clauses, errs =
    ...
    @@ List.map queries ~f:(fun pred -> pred, BindingPatt.mk_free pred)
  in
  match errs with
  | [] -> Ok Adorned.Program.(program ~cstrs clauses queries)
  | _ -> Error errs
;;
