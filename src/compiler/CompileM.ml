...
let is_name_already_bound name =
  map ~f:(fun ty_env ->
      ...
      @@ Typecheck.TypingEnv.find_name ty_env ~name)
...
let set_typing_of name typing =
  ...
  modify (fun st ->
      { st with
        typing_env =
          Typecheck.TypingEnv.update_pred_typing_exn ty_env ~name ~typing
      })
;;
...
let get_param_ty name =
  get_typing_env
  >>= fun env -> return @@ Typecheck.TypingEnv.find_param env ~name
;;

let get_pred_info name =
  get_typing_env
  >>= fun env -> return @@ Typecheck.TypingEnv.find_pred env ~name
;;
  ...
  map
    ~f:
      (Option.value_map
         ...
         ~f:Typecheck.TypingEnv.(fun { cstr; _ } -> cstr))
  @@ get_pred_info name
;;
...
let set_pred_constraint name cstr =
  ...
  >>= fun env ->
  set_typing_env
  @@ Typecheck.TypingEnv.update_pred_constraint_exn env ~name ~cstr
;;
